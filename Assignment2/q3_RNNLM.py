...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    inputs = [tf.squeeze(x, [1])
              for x in tf.split(inputs, self.config.num_steps, axis=1)]
    ### END YOUR CODE
    return inputs
  ...
  def add_loss_op(self, output):
    ...
    cross_entropy = sequence_loss(output, self.labels_placeholder, all_ones)
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN') as scope:
      ...
      for step, input in enumerate(inputs):
        ...
        b1 = tf.get_variable(
            'b1', [self.config.hidden_size])
        state = tf.nn.sigmoid(
            tf.matmul(state, H) + tf.matmul(input, I) + b1)
