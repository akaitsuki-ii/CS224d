...
class RNN_Model():
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            curr_node_tensor = tf.expand_dims(tf.nn.embedding_lookup(embedding, word_id), 0)
            ### END YOUR CODE
        else:
            node_tensors.update(self.add_model(node.left))
            node_tensors.update(self.add_model(node.right))
            ### YOUR CODE HERE
            child_node_tensor = tf.concat([node_tensors[node.left], node_tensors[node.right]], axis=1)
            curr_node_tensor = tf.nn.relu(tf.matmul(child_node_tensor, W1) + b1)
            ### END YOUR CODE
    ...
    def loss(self, logits, labels):
        ...
        loss = cross_entropy + self.config.l2 * l2_loss
        # END YOUR CODE
        return loss
    ...
    def train(self, verbose=True):
        ...
        for epoch in xrange(self.config.max_epochs):
            ...
            if val_loss < best_val_loss:
                 ...
                 for src in os.listdir('./weights'):
                     prefix = '%s.temp'%self.config.model_name
                     if src.startswith(prefix):
                         postfix = src[len(prefix):]
                         dst = self.config.model_name + postfix
                         shutil.copyfile('./weights/%s'%src, './weights/%s'%dst)
